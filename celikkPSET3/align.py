...
def main():
    ...
    # use BioPython to obtain sequence from fasta file
    ...
    v = v.lower()
    w = w.lower()
    # read the scoring file, consider only the second line, split by tabs
    f = open(scoreFile, 'r')
    fil = f.read()
    nums = fil.split("\n")[1]
    nums = nums.split("\t")
    ...
    delta[3] = delta[2]
    delta[2] = tempNum
    # depending on passed option, choose type of alignment to perform
    if option == 0:
